...
def ransac(data, model, n, k, t, d, debug=False, return_all=False):
    ...
    return bestfit
    """
    iterations = 0  # 迭代次数初始为0
    bestfit = None  # 最佳匹配为否，
    besterr = np.inf  # 误差，设置默认值
    best_inlier_idxs = None  # 最佳匹配的内群idxs
    while iterations < k:
...
class LinearLeastSquareModel:
    ...
    def __init__(self, input_columns, output_columns, debug=False):  # 构造函数，它在类实例化（创建对象）的过程中自动被调用，用于初始化对象的属性和执行其他必要的设置。
...
def test():
    ...
    n_inputs = 1  # 输入变量个数
    n_outputs = 1  # 输出变量个数
    A_exact = 20 * np.random.random((n_samples, n_inputs))  # 随机生成0-20之间的500个数据:行向量：x
    perfect_fit = 60 * np.random.normal(size=(n_inputs, n_outputs))  # 随机线性度，即随机生成一个斜率：k
    B_exact = sp.dot(A_exact, perfect_fit)  # y = x * k

    # 加入高斯噪声,最小二乘能很好的处理，500个数据是添加高斯噪声后的原始数据
    A_noisy = A_exact + np.random.normal(size=A_exact.shape)  # 500 * 1行向量,代表Xi
    B_noisy = B_exact + np.random.normal(size=B_exact.shape)  # 500 * 1行向量,代表Yi

    if 1:  # 含义是执行完上面代码才会执行以下代码，如果上面代码执行有问题则不会执行下面代码
        # 添加"局外点"
        n_outliers = 100
        ...
        A_noisy[outlier_idxs] = 20 * np.random.random(
            (n_outliers, n_inputs))  # 加入噪声和局外点的Xi：A_noisy是（500，2）包含高斯噪声的数据，现将其中outlier_idxs个数据重置为局外点
    ...
    input_columns = range(n_inputs)  # 数组all_data(500,2)的第一列x:0
    output_columns = [n_inputs + i for i in range(n_outputs)]  # 数组all_data(500,2)最后一列y:1
    debug = False
    model = LinearLeastSquareModel(input_columns, output_columns, debug=debug)  # 类的实例化:用最小二乘生成已知模型,绿线
